...
library OracleLibrary {
    /// @notice Calculates time-weighted means of tick and liquidity for a given Galador V3 pool
        ...
        returns (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity)
    {
        ...
        ) = IGaladorV3Pool(pool).observe(secondsAgos);
    ...
    /// @param pool Address of Galador V3 pool that we want to observe
    ...
    function getOldestObservationSecondsAgo(
        ...
        ) = IGaladorV3Pool(pool).slot0();
        require(observationCardinality > 0, "NI");

        (uint32 observationTimestamp, , , bool initialized) = IGaladorV3Pool(
            pool
        ...
        if (!initialized) {
            (observationTimestamp, , , ) = IGaladorV3Pool(pool).observations(0);
        }
    ...
    /// @param pool Address of Galador V3 pool
    ...
    function getBlockStartingTickAndLiquidity(
        ...
        ) = IGaladorV3Pool(pool).observations(observationIndex);
        if (observationTimestamp != uint32(block.timestamp)) {
            return (tick, IGaladorV3Pool(pool).liquidity());
        }
        ...
        ) = IGaladorV3Pool(pool).observations(prevIndex);
