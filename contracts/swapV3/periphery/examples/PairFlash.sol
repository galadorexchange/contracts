...
import "../../core/interfaces/callback/IGaladorV3FlashCallback.sol";
...
/// @notice An example contract using the Galador V3 flash function
contract PairFlash is IGaladorV3FlashCallback, PeripheryPayments {
    ...
    constructor(
        ...
        address _WETH9,
        bytes32 _codeHash
    ) PeripheryImmutableState(_deployer, _factory, _WETH9, _codeHash) {
    ...
    function galadorV3FlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data
    ) external override {
        ...
        CallbackValidation.verifyCallback(deployer, decoded.poolKey, codeHash);
    ...
    /// @notice Calls the pools flash function with data needed in `GaladorV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        ...
        IGaladorV3Pool pool = IGaladorV3Pool(
            PoolAddress.computeAddress(deployer, poolKey, codeHash)
        );
