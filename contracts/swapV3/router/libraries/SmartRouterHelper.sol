...
interface IGaladorV3PoolDeployer {
...
library SmartRouterHelper {
    ...
    function sortTokens(
        ...
        // require(tokenA != tokenB, "TokenA is tokenB");
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        // require(token0 != address(0), "Can't be zero lol");
    }
    ...
    function pairFor(
        ...
        pair = IGaladorV2Factory(factory).getPair(token0, token1);
    }
    ...
    /// @param deployer The Galador V3 deployer contract address
    ...
    function computeAddress(
        ...
        pool = IGaladorV3Factory(
            IGaladorV3PoolDeployer(deployer).factoryAddress()
        ).getPool(key.token0, key.token1, key.fee);
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) public view returns (IGaladorV3Pool) {
        return
            IGaladorV3Pool(
    ...
    /// @notice Returns the address of a valid Galador V3 Pool
    /// @param deployer The contract address of the Galador V3 deployer
    ...
    function verifyCallback(
        ...
        address tokenB,
        uint24 fee
    ) public view returns (IGaladorV3Pool pool) {
    ...
    function verifyCallback(
        ...
        pool = IGaladorV3Pool(computeAddress(deployer, poolKey));
