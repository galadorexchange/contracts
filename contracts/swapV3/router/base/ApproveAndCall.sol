...
/// @notice Allows callers to approve the Galador V3 position manager from this contract,
...
abstract contract ApproveAndCall is IApproveAndCall, ImmutableState {
    function tryApprove(address token, uint256 amount) private returns (bool) {
        (bool success, bytes memory data) = token.call(
            abi.encodeWithSelector(
                IERC20Ozep342.approve.selector,
                positionManager,
                amount
            )
        );
    ...
    function getApprovalType(
        ...
        if (
            IERC20Ozep342(token).allowance(address(this), positionManager) >=
            amount
    ...
    function balanceOf(address token) private view returns (uint256) {
        return IERC20Ozep342(token).balanceOf(address(this));
    }
