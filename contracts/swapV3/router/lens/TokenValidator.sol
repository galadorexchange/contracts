...
import "../../v2/interfaces/IGaladorCallee.sol";
...
contract TokenValidator is ITokenValidator, IGaladorCallee, ImmutableState {
    ...
    function _validate(
        ...
        (, bytes memory returnData) = address(pairAddress).call(
            abi.encodeWithSelector(IGaladorPair.token0.selector)
        );
        ...
        uint256 balanceBeforeLoan = IERC20Ozep342(token).balanceOf(
            address(this)
        );

        IGaladorPair pair = IGaladorPair(pairAddress);
    ...
    function uniswapV2Call(
        ...
        IGaladorPair pair = IGaladorPair(msg.sender);
        (address token0, address token1) = (pair.token0(), pair.token1());

        IERC20Ozep342 tokenBorrowed = IERC20Ozep342(
            amount0 > 0 ? token0 : token1
        );
