...
// import bn as bnjs from 'bignumber.js'      //  ← here we use BigNumber pure to give us more control of precision
import hre, { ethers } from "hardhat"
import { exec } from "child_process"
import fs from "fs"
import path from "path"
import jsYaml from "js-yaml"
const bnjs = require("bignumber.js").default
...
export async function deploy(contractName: string, args: any[] = [], libraries: any = {}): Promise<string> {
    const contract = await ethers.getContractFactory(contractName, libraries ? { libraries: libraries } : {})
...
export function format(x: number, decimals: number = 18) {
